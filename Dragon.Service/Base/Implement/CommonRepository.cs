...
namespace Dragon.Service.Base.Implement
{
    ...
    public class CommonRepository : ICommonRepository
    {
        ...
        /// <param name="id"></param>
        /// <returns></returns>
        public ResultView GetModel<T>(int id, IDbTransaction transaction = null, int? commandTimeout = default(int?))
        {
            try
            {
                IBaseRepository<T> db = new BaseRepository<T>();
                var model = db.GetModel(id, transaction, commandTimeout);
                if (model != null)
                {
                    return new ResultView { Info = model, Result = 1, ResultMessage = "请求成功" };
                }
        ...
        /// <param name="id"></param>
        /// <returns></returns>
        public ResultView Delete<T>(int id, IDbTransaction transaction = null, int? commandTimeout = default(int?))
        {
            try
            {
                IBaseRepository<T> db = new BaseRepository<T>();
        ...
        public ResultView Insert<T>(T entity, IDbTransaction transaction = null, int? commandTimeout = default(int?))
        {
            ...
            {            
                IBaseRepository<T> db = new BaseRepository<T>();                
                var result = db.Add(entity, transaction, commandTimeout);
                if (result>0)
                {
        ...
        /// <param name="entity"></param>
        /// <returns></returns>
        public ResultView Update<T>(T entity, IDbTransaction transaction = null, int? commandTimeout = default(int?))
        {
            try
            {
                IBaseRepository<T> db = new BaseRepository<T>();
                var result = db.Update(entity, transaction, commandTimeout);
                if (result)
                {
